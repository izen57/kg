...
def check_polygon():
	if len(verteces_list) < 3:
		return False
	sign = 1 if vect_mul(get_vect(verteces_list[1], verteces_list[2]), get_vect(verteces_list[0], verteces_list[1])) > 0 else -1
	for i in range(3, len(verteces_list)):
		if sign * vect_mul(get_vect(verteces_list[i - 1], verteces_list[i]), get_vect(verteces_list[i - 2], verteces_list[i - 1])) < 0:
			return False
...
cutter_color_label = tk.Label(data_frame, text = "Цвет регулярного отсекателя", font = ("Consolas", 14), bg = cfg.MAIN_COLOUR,
...
cutter_btn = tk.Button(data_frame, text = "Добавить вершину", font = ("Consolas", 14), bg = cfg.MAIN_COLOUR, fg = cfg.ADD_COLOUR,
	command = read_cutter_vertex, activebackground = cfg.ADD_COLOUR, activeforeground = cfg.MAIN_COLOUR)
close_btn = tk.Button(data_frame, text = "Замкнуть отсекатель", font = ("Consolas", 14), bg = cfg.MAIN_COLOUR, fg = cfg.ADD_COLOUR,
	command = lambda: return_click(0), activebackground = cfg.ADD_COLOUR, activeforeground = cfg.MAIN_COLOUR)
...
cutter_color_btn = tk.Button(data_frame, text = "", font = ("Consolas", 14), bg = cutter_color, command = change_cutter_color, relief = tk.GROOVE)
...
cutter_y_entry.place(x = cfg.DATA_WIDTH // 2, y = cfg.DATA_HEIGHT * offset // cfg.ROWS, width = cfg.DATA_WIDTH // 2, height = cfg.DATA_HEIGHT // cfg.ROWS)
