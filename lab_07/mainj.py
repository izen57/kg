...
def erase_inside():
	canvas.create_rectangle(rect[cfg.LEFT], rect[cfg.TOP], rect[cfg.RIGHT], rect[cfg.BOTTOM], fill = cfg.CANVAS_COLOUR, outline = rect_color)
...
def read_rect():
	new_rect(int(rect_left_entry.get()), int(rect_up_entry.get()), int(rect_right_entry.get()), int(rect_down_entry.get()))
...
rect_color_label = tk.Label(data_frame, text = "Цвет регулярного отсекателя", font = ("Consolas", 14), bg = cfg.MAIN_COLOUR,
