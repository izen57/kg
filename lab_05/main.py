...
def get_time():
	...
	solve()
	mb.showinfo("Время.", f"Время построения: {time.time() - start_time: 8.7f} секунд")
...
def mark_part(vertex_list, extrems):
	for j in range(len(vertex_list) - 1):
		for i in range(len(vertex_list[j])):
			print([[vertex_list[j][i].x, vertex_list[j][i].y], [vertex_list[j][(i + 1) % len(vertex_list[j])].x, vertex_list[j][(i + 1) % len(vertex_list[j])].y]],
				[i in extrems[j], (i + 1) % len(vertex_list[j]) in extrems[j]])
			mark_all_intersections([[vertex_list[j][i].x, vertex_list[j][i].y], [vertex_list[j][(i + 1) % len(vertex_list[j])].x,
				vertex_list[j][(i + 1) % len(vertex_list[j])].y]], [i in extrems[j], (i + 1) % len(vertex_list[j]) in extrems[j]])
...
colour_label = tk.Label(data_frame, text = "Цвет", font = ("Consolas", 14), bg = cfg.MAIN_COLOUR, fg = cfg.ADD_COLOUR, relief = tk.GROOVE)
